...
//=============================================================================
template <class T>
void outputMatrix(std::vector<std::vector<T> > x_)
{
	for(auto i:x_) { for(auto ii:i) { printf("%d ", ii); } printf("\n"); }
}
...
void surfaces()
{
	...
	RF.ReadSurfaceFile("Data2/kb/surface_.txt",&rot,&peq,&bmin,&bmid,&bmax);
...
void transitions()
{
	...
	//			if (obj == "KNF") { idxs = {9}; }
		...
		if(1) // AC()tion
		{
			transition.clear();
			transition.resize(s);
			for(auto &i: transition) { i.resize(s); }
			for(auto i : idxs)
			{
		...
		if(1) // object
		{
			...
			for(auto &i: transition) { i.resize(os->OSLabelList().size()); }
			for(auto i : idxs)
			{
				for(auto ii : (*label_list)[i])
				{
		...
		if(1) // obj_action
		{
			...
			for(auto i : idxs)
			{
				for(auto ii : (*label_list)[i])
				{
					if (ii=="MOVE"||ii=="RELEASE") { continue; }
					for(int iii=g1; iii<g2+1; iii++)
					{
						if(kb->AL()[iii]==ii)
						{
							transition[os->LAOSMap()[ii]][iii-g1] += 1;
						}
					}
				}
			}
			...
			for(auto i : idxs)
			{
				for(auto ii : (*label_list)[i])
				{
					...
					for(int iii=g1;iii<g2+1;iii++)
					{
						if(kb->AL()[iii]==ii)
						{
							transition[iii-g1][os->LAOSMap()[ii]] += 1; break;
						}
					}
				}
			}
